...
<body>
<nav class="Hui-breadcrumb"><i class="icon-home"></i> 首页 <span class="c-gray en">&gt;</span> 订单中心
    <span class="c-gray en">&gt;</span> 服务订单管理
    <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" >
        <i class="icon-refresh"></i></a></nav>
<form action="${ctx}/serviceOrder/list"  method="post" class="searchform" id="searchform">
    <div class="pd-20">
        <div class="text-l">
            <table width="100%" border="0" cellspacing="3" cellpadding="0">
                <tr>
                    <td width="9%" height="44">服务订单编号：</td>
                    ...
                    <td width="5%">联系人：</td>
                    <td width="17%"><input type="text" class="input-text" style="width:150px" placeholder="联系人"
                                           value="${entity.contactsName}" id="input4" name="contactsName"></td>
                    <td width="4%">状态：</td>
                    <td width="15%"><select class="select" name="status" size="1" style="width:100px">
                        <option value="">全部</option>
                        <option value="1" <c:if test="${entity.status == 1}">selected</c:if>>待确认</option>
                        <option value="2" <c:if test="${entity.status == 2}">selected</c:if>>待分配</option>
                        <option value="3" <c:if test="${entity.status == 3}">selected</c:if>>执行中</option>
                        <option value="4" <c:if test="${entity.status == 4}">selected</c:if>>已执行</option>
                        <option value="5" <c:if test="${entity.status == 5}">selected</c:if>>已取消</option>
                        <option value="6" <c:if test="${entity.status == 6}">selected</c:if>>已中止</option>
                    </select></td>
                </tr>
                <tr>
                    ...
                    <td>委托服务：</td>
                    <td>
                        <select class="select" name="orderService" size="1" style="width:100px">
                            <option value="">全部</option>
                            <option value="1" <c:if test="${entity.orderService == '出口代理'}">selected</c:if>>出口代理</option>
                            <option value="2" <c:if test="${entity.orderService == '出口报检'}">selected</c:if>>出口报检</option>
                            <option value="3" <c:if test="${entity.orderService == '出口报关'}">selected</c:if>>出口报关</option>
                            <option value="4" <c:if test="${entity.orderService == '俄境清关'}">selected</c:if>>俄境清关</option>
                            <option value="5" <c:if test="${entity.orderService == '国内运输'}">selected</c:if>>国内运输</option>
                            <option value="6" <c:if test="${entity.orderService == '俄境运输'}">selected</c:if>>俄境运输</option>
                            <option value="7" <c:if test="${entity.orderService == '国内仓储'}">selected</c:if>>国内仓储</option>
                            <option value="8" <c:if test="${entity.orderService == '俄境仓储'}">selected</c:if>>俄境仓储</option>
                        </select>
                    </td>
        ...
        <div class="cl pd-5 bg-1 bk-gray mt-20">
            <span class="l"><a href="${ctx}/serviceOrder/add" id="addService" class="btn btn-primary radius"><i class="icon-plus"></i> 新增服务订单</a></span>
        ...
        <table class="table table-border table-bordered table-hover table-bg table-sort">
            ...
            <c:forEach items="${orderList}" var="order">
                <tr class="text-c">
                    <td><a href="${ctx}/serviceOrder/${order.srvOrderId}/edit">${order.srvOrderCode}</a></td>
                    <c:if test="${order.countryType ==1}">
                        <td>${order.orgNameCn}</td>
                        <td>${order.userNameCn} - ${order.mobile}</td>
                    </c:if>
                    <c:if test="${order.countryType ==2}">
                        <td>${order.orgNameRu}</td>
                        <td>${order.userNameFirst}&nbsp;${order.userNameLast} - ${order.email}</td>
                    </c:if>
                    </td>
                    <td>${order.orderService}</td>
                    <td><odpf:formatDate style='DATE_HOUR_SECOND' date='${order.createTime}'/></td>
