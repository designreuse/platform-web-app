...
var cropUpload = function (cropOption) {
  ...
  var cropBtnWidth = $cropBtn.data("width")|| 200;
  var cropBtnHeight = $cropBtn.data("height")|| 200;
  var cropRatio = cropBtnWidth / cropBtnHeight;
  var isCrop = cropOption.isCrop || "true";
//    console.log(cropOption);
  ...
  function imgCloud(cropOption){
    $(".img-crop-select").off('click').on('click', function(){
    ...
    $(".img-cloud-select").off('click').on("click", function(){
//        console.log(cropOption);
      cropOption.imgCLoudSelect();
      $(".img-select").removeClass("btn-primary").addClass("btn-default");
      $(this).addClass("btn-primary");
  ...
  function upLoadImg(cropOption) {
    ...
    var uploader = WebUploader.create({
      ...
      swf: window.ctx+"resources/js/uploadCrop/Uploader.swf",
      accept: {
        title: 'Images',
      ...
      server: window.ctx+'/aldUploadNew/uploadImgExpert'
    });
      ...
      if (file.getStatus() === 'invalid') {
        ...
        uploader.makeThumb(file, function (error, src) {
          ...
          if (isSupportBase64) {
            ...
            imgPreview = $('<img class="img-responsive" src="' + src + '">');
            ...
            $.ajax(window.ctx+"resources/js/uploadCrop/preview.php", {
              method: 'POST',
              data: src,
              dataType: 'json'
...
//      console.log('uploadStart');
      if(isCrop == "true"){
        uploader.option('formData', {
          xPoint: parseInt(cropDate.x),
          yPoint: parseInt(cropDate.y),
          width: parseInt(cropDate.width),
          height: parseInt(cropDate.height),
          rPorint: parseInt(cropDate.rotate)
        });
      }
      /*data.x = cropDate.x;
       data.y = cropDate.y;
...
//      console.log(response);
      ...
      $upload.show();
      $uploadRetry.hide();
      if(cropOption.uploadSuccess(file, response)){
        cropOption.uploadSuccess(file, response);
      }
    });
    // 文件上传失败，显示上传出错
...
//      console.log(reason);
      ...
      if(cropOption.uploadError(file, reason)){
